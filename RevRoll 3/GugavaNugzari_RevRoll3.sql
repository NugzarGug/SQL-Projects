...
select
u.user_id,
count(s.trip_id) as flight_count
from users as u
...
user_id,
departure_time,
  lag(departure_time, 1) over (partition by user_id order by departure_time asc) as prev_dep_time
from user_time)
select user_id,
max(case when prev_dep_time is null then 0 else  extract(day from  departure_time - prev_dep_time) end ) as date_difference
from combined_times
group by 1;
